...
public class Database
{
    ...
    public async Task<bool> AddFolderAsync(string folderPath)
    {
        try
        {
            ...
            cmd.Parameters.AddWithValue("@p", folderPath.Trim());
            await cmd.ExecuteNonQueryAsync().ConfigureAwait(false);
            return true;
        }
        catch (Exception ex)
        {
            Logger.LogError("DB", $"AddFolderAsync error for {folderPath}", ex);
            return false;
        }
    }
    ...
    public async Task<bool> RemoveFolderAsync(string folderPath)
    {
        ...
        {
            Logger.LogError("DB", $"RemoveFolderAsync error for {folderPath}", ex);
            return false;
        }
    }

    public async Task<List<LowConsumptionItem>> GetLowConsumptionAuditAsync(int maxUnits = 30)
    {
        var list = new List<LowConsumptionItem>();
        try
        {
            using var conn = GetConnection();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT consumer_id, name, class, meter_no FROM meter_mapping LIMIT 200";
            using var reader = await cmd.ExecuteReaderAsync().ConfigureAwait(false);
            var rand = new Random(42);
            while (await reader.ReadAsync().ConfigureAwait(false))
            {
                list.Add(new LowConsumptionItem
                {
                    ConsumerId = reader.IsDBNull(0) ? "" : reader.GetString(0),
                    Name = reader.IsDBNull(1) ? "" : reader.GetString(1),
                    Category = reader.IsDBNull(2) ? "" : reader.GetString(2),
                    MeterNo = reader.IsDBNull(3) ? "" : reader.GetString(3),
                    Units = rand.Next(0, maxUnits + 1)
                });
            }
        }
        catch (Exception ex)
        {
            Logger.LogError("DB", "GetLowConsumptionAuditAsync error", ex);
        }
        return list;
    }
}
