...
public class Database
{
    ...
    public async Task<(bool Success, string Message)> AddAdditionalFolderAsync(string path)
    {
        if (string.IsNullOrWhiteSpace(path)) return (false, "Path cannot be empty");
        path = NormalizePath(path);
        try
        {
            using var conn = GetConnection();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = "INSERT OR IGNORE INTO additional_folders (folder_path) VALUES (@path);";
