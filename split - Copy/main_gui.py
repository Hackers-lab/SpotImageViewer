...
def add_new_note_option():
    def save_opt():
        ...
        if new_opt:
            current_opts = utils.load_note_options()
            if new_opt not in current_opts:
                current_opts.append(new_opt)
                try:
                    with open(config.NOTE_TXT, "w") as f:
                        f.write("\n".join(current_opts))
                    combo_notes['values'] = current_opts
...
def search_consumer():
    ...
    try:
        ...
        save_search_history("consumer_ids", cid)
...
def save_search_history(key, val):
    try:
        if os.path.exists(config.SEARCHED_LISTS_FILE):
            with open(config.SEARCHED_LISTS_FILE, 'r') as f: d = json.load(f)
        else: d = {"consumer_ids": [], "meter_numbers": []}
        if val in d[key]: d[key].remove(val)
        d[key].append(val)
        if len(d[key]) > 10: d[key] = d[key][-10:]
        with open(config.SEARCHED_LISTS_FILE, 'w') as f: json.dump(d, f)
    except: pass
...
def search_meter():
    m = entry_meter_number.get().strip().lower()
    if os.path.exists(config.JSON_FILE):
        try:
            with open(config.JSON_FILE, "r") as f: d = json.load(f)
            for k, v in d.items():
                if v.get("meter_no", "").lower() == m:
                    entry_consumer_id.delete(0, tk.END)
                    entry_consumer_id.insert(0, k)
                    search_consumer()
                    save_search_history("meter_numbers", m)
                    return
        except: pass
...
def update_meter_list_threaded():
    ...
    def worker(file_path):
        ...
        try:
            ...
            d = {r[0].strip(): {"meter_no": r[1].strip()} for _, r in df.iterrows()}
            utils.console_log(f"WORKER: Processed {len(d)} rows.")
            
            with open(config.JSON_FILE, "w") as f: json.dump(d, f)
            utils.console_log("WORKER: JSON saved.")
