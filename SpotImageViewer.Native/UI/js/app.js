...
function updateAppCounts(totalImages, consumerCount, availableImages) {
  if (totalImages !== null && totalImages !== undefined) {
    ...
    const dotEl = document.getElementById('statImagesDot');

    let displayCount = totalImages;
    let titleText = `${Number(totalImages).toLocaleString()} total spot images indexed`;
    let isReduced = false;

    if (availableImages !== undefined && availableImages !== null) {
      displayCount = availableImages;
      if (availableImages < totalImages) {
        isReduced = true;
        titleText = `${Number(availableImages).toLocaleString()} accessible images (${Number(totalImages - availableImages).toLocaleString()} offline from disconnected folders — click to view)`;
      } else {
        titleText = `${Number(availableImages).toLocaleString()} spot images (all folders connected & accessible)`;
      }
    }

    if (statEl) {
      statEl.innerText = Number(displayCount).toLocaleString();
      statEl.title = titleText;
      if (statEl.parentElement) statEl.parentElement.title = titleText;
    }
    if (dotEl) {
      if (isReduced) {
        dotEl.className = "w-2 h-2 rounded-full bg-amber-500 animate-pulse shrink-0";
      } else {
        dotEl.className = "w-2 h-2 rounded-full bg-emerald-500 animate-pulse shrink-0";
      }
    }
    if (idxEl) idxEl.innerText = totalImages;
    try {
      localStorage.setItem('siv_cached_total_images', String(totalImages));
      if (availableImages !== undefined && availableImages !== null) {
        localStorage.setItem('siv_cached_available_images', String(availableImages));
      }
    } catch (e) {}
  }
...
async function initApp() {
  ...
  try {
    const savedImgCount = localStorage.getItem('siv_cached_available_images') || localStorage.getItem('siv_cached_total_images');
  ...
  if (info && info.total_images !== undefined) {
    updateAppCounts(info.total_images, info.consumer_count, info.available_images);
  }
